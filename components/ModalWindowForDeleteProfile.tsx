...
const ModalWindowForDeleteProfile: FC<ModalWindowForDeleteProfileProps> = ({
    ...
    const { listImageUrlUser } = useAppSelector(state => state.DeleteProfileSlice)
    ...
    const deletePhotoProfile = async () => {
        ...
        await dispatch(fetchImageUrlUser(id))
    ...
    useEffect(() => {
        if (listImageUrlUser.length > 0) {
            const listImageUrl: string[] = [];
            listImageUrlUser.map(item => listImageUrl.push(item.imageURL))
            dispatch(fetchDeleteImageUrlUser(listImageUrl))
            return;
        }
    }, [listImageUrlUser.length])
